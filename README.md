...
### 2.품목 등록
매출보고서, 비품 신청서, 지출결의서 등에 들어가는 여러 품목과 다른 값들을 컨트롤러에서 Map으로 꺼내려고 했으나 여러 품목이 제대로 전달되지가 않았습니다.<br>

#### 원인 분석:
> - Map으로 받을 때 각 파라미터의 값을 따로 저장하여 콤마로 연결된 값의 전체 문자열이 하나의 키에 매핑되지 않기 때문에 값을 제대로 꺼내지 못하였습니다.<br>

#### 해결 방안:
> - List<String>을 활용하여 각 품목을 별도의 파라미터로 받아오는 방법을 선택했습니다. 예를 들어, @RequestParam("item") List<String> items와 같이 요청 파라미터를 List로 받아들이는 방식으로 구현했습니다. 이렇게 하면 같은 name값을 컨트롤러에서 쉽게 추출하여 처리할 수 있었습니다.<br>
